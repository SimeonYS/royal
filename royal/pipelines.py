...
class RoyalPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `royal`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM royal WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO royal (date, title, link, content)"
